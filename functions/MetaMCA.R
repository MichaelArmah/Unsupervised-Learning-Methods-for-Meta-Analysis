...
# MetaMCA function
MetaMCA <- function(data, study_col, yi_col, vi_col, mods, rho, num_pcs,...) {
  ...
  MetaMCA_result <- MCA(dat, ncp = num_pcs, row.w = inverse_variance_mv,...)
  
  list(results = MetaMCA_result, tau2 = tau2_hat)
  
}


# Example function call
MetaMCA_model <- MetaMCA(
  data   = data,
  study = "study_id",
  yi    = "g",
  vi    = "Vg",
