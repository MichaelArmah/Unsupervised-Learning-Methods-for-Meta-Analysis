...
# MetaPCA function
MetaPCA <- function(data, study_col, yi_col, vi_col, mods, rho, num_pcs,...) {
  ...
  MetaPCA_result <- PCA(dat, ncp = num_pcs, row.w = inverse_variance_mv,...)
  
  list(results = MetaPCA_result, tau2 = tau2_hat)
  
}


# Example function call
MetaPCA_model <- MetaPCA(
  data   = data,
  study = "study_id",
  yi    = "g",
  vi    = "Vg",
...
MetaPCA_model$tau2    # Reports tau squared for the model    
MetaPCA_model$results # Reports the principal component results from the MetaPCA
